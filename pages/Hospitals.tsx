...
const Hospitals: React.FC = () => {
    ...
    const [tempPasswordModal, setTempPasswordModal] = useState<{ isOpen: boolean; password: string; userName: string }>({ isOpen: false, password: '', userName: '' });
    ...
    const handleSaveUser = async () => {
        if (!selectedHospital || !userForm.name || !userForm.email) {
        ...
        try {
            if (isEditingUser && editingUserId) {
                ...
                // Password is auto-generated by the Edge Function
        ...
        <div className="space-y-6">
            ...
            {tempPasswordModal.isOpen && (
                <div className="fixed inset-0 z-[200] bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl max-w-md w-full border border-slate-200 dark:border-slate-700 animate-in fade-in zoom-in-95 duration-200">
                        <div className="p-8 text-center">
                            ...
                            <h3 className="text-xl font-black text-slate-900 dark:text-white mb-2">Usuário criado com sucesso!</h3>
                            <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">
                                Compartilhe a senha provisória abaixo com <strong className="text-slate-700 dark:text-slate-300">{tempPasswordModal.userName}</strong>. O usuário será obrigado a alterá-la no primeiro acesso.
                            </p>

                            <div className="bg-slate-50 dark:bg-slate-800 rounded-2xl p-4 mb-6 border border-slate-200 dark:border-slate-700">
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Senha Provisória</p>
                                <p className="text-2xl font-mono font-bold text-slate-900 dark:text-white tracking-wider select-all">
                            ...
                            <div className="flex gap-3">
                                <button
                                    ...
                                    className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl border-2 border-slate-200 dark:border-slate-700 text-sm font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
                                >
                                ...
                                <button
                                    onClick={() => {
                                        setTempPasswordModal({ isOpen: false, password: '', userName: '' });
                                        setCopied(false);
                                    }}
                                    className="flex-1 py-3 rounded-xl bg-primary text-white text-sm font-bold hover:bg-primary-hover transition-colors shadow-lg shadow-primary/20"
                                >
                                    Fechar
                                </button>
                            </div>
